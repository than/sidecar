...
Boards are per-directory. `sidecar init` refuses a board that is a symlink — the file itself, or the `.sidecar/` home it sits in — and writes nothing, because a link points two checkouts at one queue and every session's work piles into the same file. Run `sidecar init` in the other directory to give it a board of its own. The viewer still reads a symlinked board that already exists, so an existing setup keeps working.
