...
func (m model) statusBar() string {
	...
	// The name carries its parent directory so two boards that share a file
	// name — one per worktree, one per checkout — read as different files.
	// In a narrow pane that's the first thing to give: drop to the bare file
	// name, then truncate, so the bar is still exactly the pane width.
